...
static int SnesRomLoad()
{
	...
	if (LoadMenuOptions(snesOptionsDir, currentRomFilename, (char *)MENU_OPTIONS_EXT, (char*)&snesMenuOptions, sizeof(snesMenuOptions),1))
	{
		//failed to load options for game, so load the default global ones instead
		if (LoadMenuOptions(snesOptionsDir, (char *)MENU_OPTIONS_FILENAME, (char *)MENU_OPTIONS_EXT, (char*)&snesMenuOptions, sizeof(snesMenuOptions),1))
		{
...
#endif
{
	...
	while (1)
	{
		...
		if(Timer-tick>=(TIMER_1_SECOND))
		{
			...
			Frames=0;
			tick=Timer;
			sprintf(fps_display,"%lu %d",fps, skipcount);
			skipcount = 0;
			frame_ticks_total = 0;
		}
		...
		if(__autosave && ++SaveFrames >= 18000 ) //18000
		{
			char svfilename[1024];
            sprintf(svfilename, "%s%s%s-last-autosave.sv", snesSaveStateDir, DIR_SEPERATOR, currentRomFilename);
			gp_drawString(0,0,strlen("autosaving!"),(char *)"autosaving",0xFFFF,(unsigned char*)vrambuffer);
			MenuFlip();
